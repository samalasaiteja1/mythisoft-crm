...
import TechnicalManagerDashboardCharts from './TechnicalManagerDashboardCharts';
...
export default function TechnicalManagerDashboard({ departmentOverview, roleStats }) {
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      {/* Charts Section */}
      <TechnicalManagerDashboardCharts roleStats={stats} />
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <h2 className="text-base lg:text-lg font-semibold text-white flex items-center gap-2">
              <Cpu size={16} lg:size={20} className="text-cyan-400" /> Technical Manager Control Center
            </h2>
          ...
          <div className="flex flex-wrap gap-2">
            <span className="inline-flex items-center gap-1.5 px-2 lg:px-3 py-1.5 rounded-lg bg-blue-500/10 border border-blue-500/30 text-[10px] lg:text-sm text-blue-300">
              <FolderKanban size={12} lg:size={14} /> {activeProjects} active project{activeProjects === 1 ? '' : 's'}
            </span>
            <span className="inline-flex items-center gap-1.5 px-2 lg:px-3 py-1.5 rounded-lg bg-sky-500/10 border border-sky-500/30 text-[10px] lg:text-sm text-sky-300">
              <Users size={12} lg:size={14} /> {members} team member{members === 1 ? '' : 's'}
            </span>
            {pendingAcceptance > 0 && (
              <span className="inline-flex items-center gap-1.5 px-2 lg:px-3 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/30 text-[10px] lg:text-sm text-amber-300">
                <UserCheck size={12} lg:size={14} /> {pendingAcceptance} awaiting customer acceptance
              </span>
            )}
            {alerts > 0 && (
              <span className="inline-flex items-center gap-1.5 px-2 lg:px-3 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/30 text-[10px] lg:text-sm text-amber-300">
                <AlertTriangle size={12} lg:size={14} /> {alerts} need attention
              </span>
            )}
            <Link to="/notifications" className="btn-secondary text-xs lg:text-sm inline-flex items-center gap-1">
              <Bell size={12} lg:size={14} /> Notifications
            </Link>
          </div>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-[10px] lg:text-xs font-semibold uppercase tracking-wider text-gray-400">Needs Attention</h3>
          {alerts === 0 && (
            <span className="text-[10px] lg:text-xs text-green-400">All clear — nothing overdue</span>
          )}
        </div>
          ...
          {ACTION_ITEMS.map((item) => {
            ...
            return (
              <Link
                key={item.key}
                to={item.link}
                className={`p-2 lg:p-4 rounded-xl border transition-colors hover:border-myth-accent/40 ${isUrgent ? item.bg : 'border-myth-border bg-myth-surface/50'}`}
              >
                <div className="flex items-center gap-2 mb-1 lg:mb-2">
                  <Icon size={14} lg:size={16} className={item.color} />
                  <span className="text-[10px] lg:text-xs text-gray-400 leading-tight">{item.label}</span>
                </div>
                <p className={`text-xl lg:text-2xl font-bold ${isUrgent ? 'text-white' : 'text-gray-500'}`}>{value}</p>
              </Link>
            );
          })}
        </div>
      ...
      <div>
        <h3 className="text-[10px] lg:text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3">Project Health</h3>
          ...
          {PROJECT_HEALTH.map((item) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-1 lg:mb-2">
                  <Icon size={14} lg:size={16} className={item.color} />
                  <span className="text-[10px] lg:text-xs text-gray-400">{item.label}</span>
                </div>
                <p className="text-xl lg:text-2xl font-bold text-white">{formatMetric(stats[item.key])}</p>
              </Link>
            );
          })}
        </div>
        <div className="mt-3 flex flex-wrap gap-3 lg:gap-4 text-[10px] lg:text-xs text-gray-500">
          ...
          <Link to="/projects" className="text-myth-accent hover:underline inline-flex items-center gap-1">
            View all projects <ArrowRight size={10} lg:size={12} />
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
        <div className="lg:col-span-2 card">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base lg:text-lg font-semibold text-white flex items-center gap-2">
              <Layers size={14} lg:size={18} className="text-myth-accent" /> Team Snapshot
            </h3>
            <Link to="/teams/technical/overview" className="text-xs lg:text-sm text-myth-accent hover:underline">All teams →</Link>
          </div>
            ...
            {TEAM_METRICS.map((metric) => {
              ...
              return (
                <Link
                  key={metric.key}
                  to={metric.link}
                  className="p-2 lg:p-3 rounded-lg bg-myth-surface/50 border border-myth-border hover:border-myth-accent/30 transition-colors text-center"
                >
                  <Icon size={14} lg:size={16} className="text-myth-accent mx-auto mb-1" />
                  <p className="text-base lg:text-lg font-bold text-white">{formatMetric(stats[metric.key], metric.format)}</p>
                  <p className="text-[10px] lg:text-xs text-gray-500 mt-0.5">{metric.label}</p>
                </Link>
              );
            })}
          </div>
          ...
          {deptInfo?.teams?.length > 0 ? (
            <div className="space-y-2">
              {deptInfo.teams.map((t) => (
                <div
                  key={t.code}
                  className="flex items-center justify-between text-xs lg:text-sm px-2 lg:px-3 py-2 rounded-lg bg-myth-surface/30 border border-myth-border/50"
                >
                  <div>
                    <span className="text-myth-accent font-mono text-[10px] lg:text-xs">{t.code}</span>
            ...
            <p className="text-xs lg:text-sm text-gray-500 py-4 text-center">
              No teams yet.{' '}
        ...
        <div className="card">
          <h3 className="text-base lg:text-lg font-semibold text-white mb-4 flex items-center gap-2">
            <Plus size={14} lg:size={18} className="text-myth-accent" /> Quick Actions
          </h3>
          <div className="space-y-2">
            {QUICK_ACTIONS.map((action) => {
              ...
              return (
                <Link
                  key={action.path}
                  to={action.path}
                  className={`flex items-center gap-2 lg:gap-3 p-2 lg:p-3 rounded-lg border transition-colors ${
                    action.primary
                ...
                >
                  <Icon size={14} lg:size={18} className={action.primary ? 'text-myth-accent' : 'text-gray-400'} />
          ...
          <div className="mt-4 lg:mt-5 pt-4 border-t border-myth-border">
            <p className="text-[10px] lg:text-xs font-semibold uppercase text-gray-500 mb-2 flex items-center gap-1">
              <LayoutGrid size={10} lg:size={12} /> Tasks & Delivery
            </p>
            <div className="grid grid-cols-2 gap-1.5">
              {DELIVERY_LINKS.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className="text-[10px] lg:text-xs text-myth-accent hover:underline py-1.5 px-2 rounded hover:bg-myth-surface/50"
                >
                  {link.label}
                </Link>
              ))}
      ...
      {/* Compact workflow */}
      <div className="card">
        <h3 className="text-[10px] lg:text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3">Delivery Workflow</h3>
        <div className="flex flex-wrap items-center gap-1">
          {TECH_MANAGER_WORKFLOW.slice(0, 8).map((step, i, arr) => (
            <div key={step} className="flex items-center gap-1">
              <span className="text-[10px] lg:text-xs text-gray-300 px-2 py-1 rounded bg-myth-surface/50 border border-myth-border">
                {step}
              </span>
              {i < arr.length - 1 && <span className="text-gray-600 text-[10px] lg:text-xs">→</span>}
            </div>
          ))}
          <span className="text-gray-600 text-[10px] lg:text-xs mx-1">→</span>
          <span className="text-[10px] lg:text-xs text-gray-400">Deploy → Submit</span>
        </div>
      </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base lg:text-lg font-semibold text-white flex items-center gap-2">
            <Users size={14} lg:size={18} className="text-myth-accent" /> Team Members
          </h3>
          <Link to="/teams/technical/members" className="text-xs lg:text-sm text-myth-accent hover:underline">View all →</Link>
        </div>
              ...
              {recentEmployees.length ? recentEmployees.slice(0, 8).map((e) => (
                <tr key={e._id} className="border-t border-myth-border/50">
                  ...
                  <td className="table-cell">
                    <span className="badge text-[10px] lg:text-xs capitalize bg-cyan-500/20 text-cyan-300">{e.role}</span>
                  </td>
